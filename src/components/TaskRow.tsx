...
export default function TaskRow({ task, onStatus, onDelete, darkMode: dm, onFocusTime }: TaskRowProps) {
  ...
  return (
    <div className={`rounded-2xl border px-4 sm:px-5 py-4 transition-all ${dm ? 'bg-[#221d14] border-[#3a3020]' : 'bg-white border-[#e8e0cc]'} ${task.status !== 'pending' ? 'opacity-75' : ''}`}>
      <div className="flex flex-col sm:flex-row sm:items-center gap-4">
        {/* Title + meta */}
        <div className="min-w-0 sm:w-52 sm:flex-shrink-0">
          <div className="flex items-center gap-2 mb-1">
            <span className={`w-2 h-2 rounded-full flex-shrink-0 ${statusDotColor}`} />
            <span className={`font-semibold text-sm truncate ${dm ? 'text-[#e8d9b5]' : 'text-[#2c2416]'} ${task.status === 'done' ? 'line-through' : ''}`}>
              {task.title}
            </span>
          </div>
        ...
        <div className="flex items-center flex-wrap gap-4 sm:gap-4 sm:ml-auto">
          {/* Day checkboxes */}
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`flex items-center gap-1.5 px-3 sm:px-4 py-2 rounded-xl border text-xs font-semibold transition-all ${
                task.status === 'done'
